...
app.post('/signup', async (req, res) => {
    ...
    // Assuming you have already defined the messDetailsSchema and the model

// Handle the add meal details form submission
app.post('/addMealDetails', async (req, res) => {
    const { name, location, timetable } = req.body;

    try {
        await MessDetails.create({ name, location, timetable });
...
// Assuming you have a route for rendering the add meal details form
app.get('/addMealDetailsForm', (req, res) => {
    res.render('addMealDetailsForm'); // Create a new EJS file for the form
});
// Handle the delete meal details button click
app.post('/deleteMealDetails/:id', async (req, res) => {
    const mealDetailsId = req.params.id;

    try {
        await MessDetails.findByIdAndDelete(mealDetailsId);
        res.redirect('/student');
    } catch (error) {
        console.error(error);
        res.status(500).send('Internal Server Error');
